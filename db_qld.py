...
async def insert_into_db(data):
    ...
    if not crsr.fetchone():
        try:
            crsr.execute("INSERT INTO qld_draws(current_game_number, draw, closing, 7_spot, 8_spot, 9_spot, 10_spot) VALUES (%s, %s, %s, %s, %s, %s, %s)", (current_game_number, draw_json, closing, base_seven_spot, base_eight_spot, base_nine_spot, base_ten_spot,))
            conn.commit()
...
async def continuously_check_condition(api_url):
    while True:
        ...
        if data is not None:
            ...
            difference_in_seconds = 0
            send_telegram_message(f"QLD - {data}")
